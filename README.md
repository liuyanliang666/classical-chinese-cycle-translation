...
本项目研究古文与现代汉语之间的双向翻译，并将视觉领域 CycleGAN 的 **Cycle Consistency** 思想迁移到文本生成任务中。目标不是只追求单向 BLEU，而是让译文在语义、结构和关键信息上更可逆，减少古今互译中的信息丢失和幻觉。
...
本仓库只保留可复现实验所需的代码、轻量示例数据、核心结果和图表。以下内容不纳入 Git：

- 原始数据、处理后大数据和模型 checkpoint。
- 本地 agent 计划、临时 PPT 大纲和演示截图。
- Python / Node 缓存与本地环境目录。
